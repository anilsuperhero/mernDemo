...
const Index = (props) => {
  const { title } = props;
  const { isData, customerData, dialogOpen } = useSelector((state) => ({
    isData: state.isData,
  ...
  const tableHead = [
    {
      ...
      sort: null,
      class: "fas fa-sort",
      asc: "sort=company_name&direction=asc",
      desc: "sort=company_name&direction=desc",
    },
    {
      title: "Name",
      key: "first_name",
  ...
  const handlePasswordClick = (item) => {
    dispatch(loadRequestData(item));
    history.push(`/users/customer/change-password/${item.id}`);
  };
  ...
  const handleDeleteClick = (data) => {
    if (data.action) {
      ...
      data.dialogTitle = "Delete Company";
      data.open = true;
      setDialog(data);
