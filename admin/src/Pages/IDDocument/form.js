...
const FormModal = (props) => {
  ...
  const [Idtype, setIdType] = useState("Primary");
  const { item } = props;
  ...
  const onSubmit = (data) => {
    const param = data;
    param.type = type;
    param.id_document_type = Idtype;
    param.extension = personName;
    if (param.id) {
      dispatch(submitIDDocument(param, goToPreviousPath));
    } else {
      dispatch(createIDDocument(param, goToPreviousPath));
    }
  };
  ...
  useEffect(() => {
    ...
    if (item._id) {
      ...
      setType(item.type);
      setIdType(item.id_document_type);
    }
        ...
        <Form onSubmit={handleSubmit(onSubmit)} noValidate autoComplete="off">
          ...
          <Row>
            ...
            <Col md={6}>
              ...
              <Form.Group controlId="formBasicdescription">
                <InputLabel required={true}>Id Document Type</InputLabel>
                <FormControl component="fieldset">
                  <RadioGroup
                    aria-label="type"
                    name="type"
