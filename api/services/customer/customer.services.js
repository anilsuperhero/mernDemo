    ...
    try {
      ...
      Users.findOne({ _id: id }, async function (err, result) {
        if (err || !result) {
          ...
          return res.json(responseData("USER_PASSWORD_UPDATE"))
        }
      })
    } catch (err) {
      return res.status(422).json(responseData(err.message, {}, 422))
      ...
      user.save(async function (err, result) {
        if (err) {
          ...
          if (files && files.image.name != undefined) {
            ...
            await Users.findOneAndUpdate({ _id: result._id }, user)
          }
          ...
          options.description = _.replace(options.description, "[EMAIL]", email)
          options.description = _.replace(
            options.description,
            "[PASSWORD]",
            password
    ...
    try {
      ...
      Users.findOne({ _id: id }, async function (err, result) {
        if (err || !result) {
          ...
          updateObj.company_name = company_name
          updateObj.abn_number = abn_number
          updateObj.address = address
          updateObj.address_line = address_line
          updateObj.city = city
          ...
          await Users.findOneAndUpdate(
            ...
            (err, result) => {
              ...
              return res.json(responseData("COMPANY_UPDATE"))
            }
          )
        }
      })
        ...
        async (err, result) => {
          Users.findOne({ _id: id }, async function (err, result) {
            if (err || !result) {
               ...
               * Send Email
               */
              var message = "Your account has been deactivated by administrator"
              if (parseInt(status)) {
                var message = "Your account has been activated by administrator"
              }
              ...
              options.toEmail = result.email
              sendMail(options)
              var stateMessage = "COMPANY_DEACTIVE_STATE"
              if (parseInt(status)) {
                var stateMessage = "COMPANY_ACTIVE_STATE"
              }
  ...
  delete: async (req, res) => {
    try {
      ...
      await Users.findOneAndUpdate(
        ...
        async (err, result) => {
          return res.json(responseData("COMPANY_DELETE"))
        }
      )
    } catch (err) {
      return res.status(422).json(responseData(err.message, {}, 422))
