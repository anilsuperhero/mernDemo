    ...
    try {
      ...
      Users.findOne({ _id: id }, async function (err, result) {
        if (err || !result) {
          ...
          return res.json(responseData("AUDITOR_PASSWORD_UPDATE"));
        }
      });
    } catch (err) {
      return res.status(422).json(responseData(err.message, {}, 422));
        ...
        if (err || !result) {
          ...
          await Users.findOneAndUpdate(
            ...
            (err, result) => {
              ...
              return res.json(responseData("AUDITOR_UPDATE"));
            }
          );
        }
      });
        ...
        async (err, result) => {
          Users.findOne({ _id: id }, async function (err, result) {
            if (err || !result) {
              ...
              var stateMessage = "AUDITOR_DEACTIVE_STATE";
              if (parseInt(status)) {
                var stateMessage = "AUDITOR_ACTIVE_STATE";
              }
  ...
  delete: async (req, res) => {
    try {
      ...
      await Users.findOneAndUpdate(
        ...
        async (err, result) => {
          return res.json(responseData("AUDITOR_DELETE"));
        }
      );
    } catch (err) {
      return res.status(422).json(responseData(err.message, {}, 422));
