...
module.exports = {
  index: async (req, res) => {
    try {
      ...
      var query = { isDeleted: false, isOther: true }
      if (status) {
        query.status = status
      }
      if (keyword) {
  ...
  delete: async (req, res) => {
    try {
      ...
      await DocumentType.findOneAndUpdate(
        ...
        async (err, result) => {
          return res.json(responseData("OTHER_DOCUMENT_DELETE"))
        }
      )
    } catch (err) {
      return res.status(422).json(responseData(err.message, {}, 422))
  ...
  update: async (req, res) => {
    try {
      let { title, description, type, extension, id } = req.body
      let updateObj = {}
      updateObj.title = title
      updateObj.type = type
      updateObj.extension = extension
      updateObj.description = description
      updateObj.isOther = true
      await DocumentType.findOneAndUpdate(
        { _id: id },
        updateObj,
        (err, result) => {
  ...
  process: async (req, res) => {
    try {
      ...
      await DocumentType.findOneAndUpdate(
        ...
        { status: status },
        (err, result) => {
          var stateMessage = "OTHER_DOCUMENT_DEACTIVE_STATE"
          if (parseInt(status)) {
            var stateMessage = "OTHER_DOCUMENT_ACTIVE_STATE"
          }
