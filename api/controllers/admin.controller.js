var admin_service = require("../services/admin/admin.services")
...
module.exports = {
  ...
  create: async (req, res) => {
    try {
      await admin_service.create(req, res)
    } catch (err) {
      var msg = err.message || "SOMETHING_WENT_WRONG"
      return res.status(422).json(responseData(msg, {}, 422))
    }
  ...
  Index: async (req, res) => {
    try {
      await admin_service.index(req, res)
    } catch (err) {
      var msg = err.message || "SOMETHING_WENT_WRONG!"
  ...
  update: async (req, res) => {
    try {
      await admin_service.update(req, res)
    } catch (err) {
      var msg = err.message || "SOMETHING_WENT_WRONG"
      return res.status(422).json(responseData(msg, {}, 422))
    }
  ...
  Process: async (req, res) => {
    try {
      await admin_service.process(req, res)
    } catch (err) {
      var msg = err.message || "SOMETHING_WENT_WRONG!"
      return res.json(responseData(msg, {}, 201))
    }
  ...
  Delete: async (req, res) => {
    try {
      await admin_service.delete(req, res)
    } catch (err) {
      var msg = err.message || "SOMETHING_WENT_WRONG!"
      return res.json(responseData(msg, {}, 201))
    }
